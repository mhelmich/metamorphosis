...
func BenchmarkBadgerLogAppend(b *testing.B) {
	dir := "./BenchmarkBadgerLogAppend/"
	assert.Nil(b, os.RemoveAll(dir))
	cc := newMockCopyCat()
...
func TestBadgerLogCompaction(t *testing.T) {
	...
	assert.Equal(t, numEntries+31, countLogEntries(log))
...
func TestBadgerLogSnapshotProviderConsumer(t *testing.T) {
	...
	assert.Equal(t, 61, countEntriesInState(log1))
	// HACK - compaction ran therefore we have our reserved key in there as well
	assert.Equal(t, 51, countLogEntries(log1))
	...
	assert.Nil(t, err)
	assert.Equal(t, 50, len(logSnap.Log))
	assert.Equal(t, 61, len(logSnap.State.M))
	...
	assert.Equal(t, 61, countEntriesInState(log2))
	// HACK - compaction ran therefore we have our reserved key in there as well
	assert.Equal(t, 51, countLogEntries(log2))
